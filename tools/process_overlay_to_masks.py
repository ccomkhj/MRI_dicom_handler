...
def load_series_uids_from_parquet(parquet_dir: str) -> Dict[str, int]:
    """
    Load Series Instance UIDs and their corresponding classes from parquet files.
    
    Args:
        parquet_dir: Directory containing class-organized parquet files
    
    Returns:
        Dict mapping Series UID to class number
    """
    series_class_map = {}
    ...
    for class_dir in sorted(parquet_path.glob("class=*")):
        ...
        for parquet_file in class_dir.glob("*.parquet"):
            ...
            if series_col:
                series_uids = df[series_col].dropna().unique()
                for uid in series_uids:
                    series_class_map[str(uid)] = class_num
                print(f"  Class {class_num}: Added {len(series_uids)} series UIDs from {parquet_file.name}")
            else:
                print(f"  Warning: No Series UID column in {parquet_file.name}")
                print(f"    Available columns: {list(df.columns)}")
    
    print(f"\n✓ Total unique series UIDs: {len(series_class_map)}")
    return series_class_map
...
def match_overlay_dirs_with_series(overlay_base_dir: str, 
                                   series_class_map: Dict[str, int]) -> Dict[str, Dict]:
    """
    Match overlay directories with Series Instance UIDs.
    
    Args:
        overlay_base_dir: Base directory containing overlay data
        series_class_map: Mapping of Series UID to class
    
    Returns:
        Dict mapping overlay directory to metadata (series_uid, class, patient_id)
    """
    ...
    for overlay_dir in overlay_path.iterdir():
        ...
        if series_uid and series_uid in series_class_map:
            # Extract patient ID
            patient_id_match = re.search(r'Prostate-MRI-US-Biopsy-(\d+)', overlay_dir.name)
            patient_id = patient_id_match.group(1) if patient_id_match else "unknown"
            
            matched_dirs[str(overlay_dir)] = {
                "series_uid": series_uid,
                "class": series_class_map[series_uid],
...
def process_overlay_directory(overlay_dir: Path, 
    ...
    patient_output_dir = output_base_dir / f"class={class_num}" / f"patient_{patient_id}"
    masks_dir = patient_output_dir / "masks"
    biopsies_dir = patient_output_dir / "biopsies"
    
    # Look for Data subdirectory
    data_dir = overlay_dir / "Data"
    if not data_dir.exists():
        print(f"  Warning: No Data/ subdirectory in {overlay_dir.name}")
        return stats
    ...
    for stl_file in stl_files:
        ...
        try:
            ...
            num_slices = save_voxel_slices_as_png(voxel_grid, masks_dir, prefix=mask_name)
            
            stats["slices_created"] += num_slices
            stats["stl_files_processed"] += 1
            
            print(f"      → Created {num_slices} mask slices")
    ...
    # Save biopsy coordinates as JSON
    if biopsies_data:
        biopsies_dir.mkdir(parents=True, exist_ok=True)
        biopsies_json = biopsies_dir / "biopsies.json"
        with open(biopsies_json, 'w') as f:
            json.dump(biopsies_data, f, indent=2)
        print(f"    Saved {len(biopsies_data)} biopsy annotations to {biopsies_json.name}")
    
    # Copy MRML file for reference
    mrml_files = list(overlay_dir.glob("*.mrml"))
    if mrml_files:
        patient_output_dir.mkdir(parents=True, exist_ok=True)
        shutil.copy2(mrml_files[0], patient_output_dir / "scene.mrml")
...
def main():
    ...
    output_base_dir = "data/overlay_processed"
    voxel_size = 0.5  # mm per voxel
    
    # Step 1: Load Series Instance UIDs from parquet files
    series_class_map = load_series_uids_from_parquet(parquet_dir)
    
    if not series_class_map:
        print("❌ Error: No Series UIDs found in parquet files!")
        return
    
    # Step 2: Match overlay directories with Series UIDs
    matched_dirs = match_overlay_dirs_with_series(overlay_base_dir, series_class_map)
    
    if not matched_dirs:
        print("❌ Error: No matching overlay directories found!")
        return
    
    # Step 3: Process each matched directory
    print(f"\n{'='*80}")
    ...
    for i, (overlay_dir_str, info) in enumerate(matched_dirs.items(), 1):
        overlay_dir = Path(overlay_dir_str)
        
        print(f"\n[{i}/{len(matched_dirs)}] Patient {info['patient_id']} (Class {info['class']})")
        print(f"  Directory: {overlay_dir.name[:80]}...")
    ...
    print(f"  Structure: class={{1,2,3,4}}/patient_{{ID}}/masks/")
